...
func Search(keyword string) (*District, error) {
	...
	for code, name := range ProvAndCities {
		if name == keyword {
			result = []int{code}
			break
		} else if strings.Contains(name, keyword) {
...
func isProvince(code int) bool {
	return code%10000 == 0
}

func isCity(code int) bool {
	return code%100 == 0 && code%10000 > 0
}

func getLevel(code int) int {
	var level int
	if isProvince(code) {
		level = Province
	} else if isCity(code) {
